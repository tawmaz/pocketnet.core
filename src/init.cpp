 ...
 * between pocketcoind, and pocketcoin-qt and non-server tools.
*/
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-conf=<file>", strprintf("Specify configuration file. Relative paths will be prefixed by datadir location. (default: %s)", POCKETCOIN_CONF_FILENAME), false, OptionsCategory::OPTIONS);
...
#ifndef WIN32
    gArgs.AddArg("-pid=<file>", strprintf("Specify pid file. Relative paths will be prefixed by a net-specific datadir location. (default: %s)", POCKETCOIN_PID_FILENAME), false, OptionsCategory::OPTIONS);
#else
    ...
    gArgs.AddArg("-rpcport=<port>", strprintf("Listen for secure JSON-RPC connections on <port> (default: %u, testnet: %u, regtest: %u)", defaultBaseParams->RPCPort(), testnetBaseParams->RPCPort(), regtestBaseParams->RPCPort()), false, OptionsCategory::RPC);
...
static void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("pocketcoin-loadblk");
...
static void StartWS()
{
    ...
    ws.on_message = [](std::shared_ptr<WsServer::Connection> connection, std::shared_ptr<WsServer::InMessage> in_message) {
...
bool AppInitMain()
{
    ...
    fs::path config_file_path = GetConfigFile(gArgs.GetArg("-conf", POCKETCOIN_CONF_FILENAME));
