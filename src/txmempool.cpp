...
void CTxMemPool::removeRecursive(const CTransaction& origTx, MemPoolRemovalReason reason)
{
    ...
    else
    {
        ...
        // Also remove from Transactions DB if the original transaction is present
...
static void CheckInputsAndUpdateCoins(const CTransaction& tx, CCoinsViewCache& mempoolDuplicate, const int64_t spendheight)
{
    ...
    bool fCheckResult = tx.IsCoinBase() ||
                        Consensus::CheckTxInputs(tx, state, mempoolDuplicate, spendheight, txfee, Params());
    assert(fCheckResult);
...
void CTxMemPool::RemoveStaged(setEntries& stage, bool updateDescendants, MemPoolRemovalReason reason)
{
    ...
    for (txiter it : stage)
    {
        removeHashes.emplace(it->GetTx().GetHash().GetHex());
        removeUnchecked(it, reason);
    }

    if (reason != MemPoolRemovalReason::BLOCK)
    {
        CleanSQLite(removeHashes, reason);
    }
}
...
int CTxMemPool::Expire(int64_t time)
{
    ...
    PocketDb::TransRepoInst.CleanExpiredTransactions(time);
