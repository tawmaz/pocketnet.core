...
/** Default min fee for PocketNET transactions */
static const unsigned int DEFAULT_MIN_POCKETNET_TX_FEE = 1;
...
//bool CheckBlockAdditional(CBlockIndex* pindex, const CBlock& block, BlockValidationState& state);

/** Check a block is completely valid from start to finish (only works on top of our current best block) */
bool TestBlockValidity(BlockValidationState& state, const CChainParams& chainparams, const CBlock& block, const PocketBlockRef& pocketBlock, CBlockIndex* pindexPrev, bool fCheckPOW = true, bool fCheckMerkleRoot = true) EXCLUSIVE_LOCKS_REQUIRED(cs_main);
...
public:
    ...
    bool ActivateBestChain(
        ...
        std::shared_ptr<PocketHelpers::PocketBlock> pocketBlock) LOCKS_EXCLUDED(cs_main);

    bool AcceptBlock(const std::shared_ptr<const CBlock>& pblock, const PocketBlockRef& pocketBlock, BlockValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fRequested, const FlatFilePos* dbp, bool* fNewBlock) EXCLUSIVE_LOCKS_REQUIRED(cs_main);

    // Block (dis)connection on a given view:
    DisconnectResult DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view);
    bool ConnectBlock(const CBlock& block, const PocketBlockRef& pocketBlock, BlockValidationState& state, CBlockIndex* pindex,
...
private:
    bool ActivateBestChainStep(BlockValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexMostWork, const std::shared_ptr<const CBlock>& pblock, const std::shared_ptr<PocketHelpers::PocketBlock>& pocketBlock, bool& fInvalidFound, ConnectTrace& connectTrace) EXCLUSIVE_LOCKS_REQUIRED(cs_main, m_mempool.cs);
    bool ConnectTip(BlockValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, const std::shared_ptr<PocketHelpers::PocketBlock>& pocketBlockPart, ConnectTrace& connectTrace, DisconnectedBlockTransactions& disconnectpool) EXCLUSIVE_LOCKS_REQUIRED(cs_main, m_mempool.cs);
...
public:
     ...
     * #param[in]   fReceived Block received from another node or create self?
     ...
     */
    bool ProcessNewBlockHeaders(const std::vector<CBlockHeader>& block, BlockValidationState& state, const CChainParams& chainparams, const CBlockIndex** ppindex = nullptr, const CBlockIndex** ppindexFirst = nullptr) LOCKS_EXCLUDED(cs_main);
